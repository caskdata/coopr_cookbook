...
describe 'coopr::repo' do
  ...
  describe 'rhel' do
    it 'Verifies the repository file is present and correct' do
      ...
      file('/etc/yum.repos.d/cask.repo').must_include "baseurl=#{node['coopr']['repo']['url']}"
  ...
  describe 'debian' do
    it 'Verifies the repository file is present and correct' do
      ...
      file('/etc/apt/sources.list.d/cask.list').must_include "#{node['coopr']['repo']['url']}"
    end
  end
end
